...
DEVICE = torch.device('cuda:2' if torch.cuda.is_available() else 'cpu') #set the device as GPU if available
...
print(eval['hindi'][25], translate(transformer, eval['hindi'][25]))
...
# with open(Lang+'_true.txt', 'w', encoding = 'utf-8') as f:
#   for sentence in tqdm(eval['english']):
#     f.write(sentence + '\n')

# with open(Lang+'_original_text.txt', 'w', encoding = 'utf-8') as f:
#   for sentence in tqdm(eval['hindi']):
#     f.write(sentence + '\n')

# with open(Lang+'_pred.txt', 'w', encoding = 'utf-8') as f:
#   for sentence in tqdm(eval['hindi']):
